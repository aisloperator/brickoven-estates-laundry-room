...
- **Two front-load door looks**: `makeFrontLoad(width, height, depth, bodyMat, doorStyle)` takes an optional `doorStyle` — `'round'` (default: chrome-rimmed glass circle, used for the 2 front-load washers) or `'rect'` (white rounded-rectangle panel via `roundedRectShape` + `ShapeGeometry`, used for the standalone dryer and stacked dryers). Both hinge and swing open the same way; only the visible door mesh differs. The rect panel is sized with margin (`doorW`/`doorH` well past `doorR`) so it fully covers the circular drum hole behind it even with rounded corners cut in. The drum itself stays circular either way.
