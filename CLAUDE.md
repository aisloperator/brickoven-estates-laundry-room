...
- **Fire**: `buildFlameCluster(orient, scaleMul)` is a shared helper (layered additive-blended cones, red + orange) used both for the dog's mouth-fire and for the laundry's own burning-in-place fire; `orient: 'z'` points the cones forward (breath), `orient: 'up'` points them up (ground fire).
- **Cleanup**: the laundry pile, its burn-phase flame cluster, and the pee-stream mesh are created fresh per play and disposed (`geometry.dispose()`/`material.dispose()`) at the end of their step; scorch marks left on the floor persist but are capped (oldest removed past 16) to avoid unbounded growth over a long session.
