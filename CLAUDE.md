...
- **Two front-load door looks**: `makeFrontLoad(width, height, depth, bodyMat, doorStyle)` takes an optional `doorStyle` — `'round'` (default: chrome-rimmed glass circle, used for the standalone dryer and stacked dryers) or `'rect'` (white rounded-rectangle panel via `roundedRectShape` + `ShapeGeometry`, used for the 2 front-load washers). Both hinge and swing open the same way; only the visible door mesh differs. The drum behind stays circular either way.
