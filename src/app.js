(function () {
  ...
  // Pulled back a little further and aimed a little more toward the
  // rightmost washer (near the doorway) than dead-center, while staying
  // wide enough to keep every machine in frame.
  camera.position.set(ROOM_W * 0.82, 3.3, ROOM_D * 2.15);
  ...
  // Nudged from dead room-center toward the rightmost washer (the last one
  // in washerCenters, by the doorway) to match the initial camera framing.
  controls.target.set(washerCenters[washerCenters.length - 1] - 0.2, 1.0, ROOM_D * 0.42);
