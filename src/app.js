(function () {
  ...
  // 4 stacks in the back-right corner (the only corner with two actual
  // walls — back wall meets right wall here, just past the doorway). One
  // tight in the corner itself, one a few inches out along the washer wall,
  // one a few inches out along the dryer wall, and one further along the
  // dryer wall. Positions are hand-fit against the real dryer footprints
  // (dryerCenters[0] spans roughly z 1.0-1.72, [1] 1.75-2.47, [2] 2.49-3.21
  // at this wall) so none of the stacks clip into a dryer cabinet.
  var cornerStackSpecs = [
    { x: ROOM_W - 0.35, z: 0.30, count: 23 },  // tight in the corner
    { x: ROOM_W - 0.81, z: 0.30, count: 23 },  // a few inches along the washer wall
    { x: ROOM_W - 0.35, z: 0.70, count: 22 },  // a few inches along the dryer wall
    { x: ROOM_W - 0.35, z: 3.70, count: 22 }   // further along the dryer wall
  ];
