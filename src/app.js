(function () {
  ...
  function createPosterTexture() {
    // Portrait, matching the 3ft x 4ft (3:4) poster it's mapped onto.
    var cnv = makeCanvas(384, 512);
    var ctx = cnv.getContext('2d');
    ctx.fillStyle = '#e4f1fb';
    ctx.fillRect(0, 0, 384, 512);
    ctx.strokeStyle = '#a9c6db';
    ctx.lineWidth = 6;
  ...
  function makeFrontLoad(width, height, depth, bodyMat, doorStyle, extraPanelH, machineNumber, label) {
    ...
    var panel = new THREE.Mesh(new THREE.BoxGeometry(width * 0.92, panelH, panelThickness), doorWhiteMat);
  ...
  function makeStackedDryerColumn(width, depth, topNumber, topLabel, bottomNumber, bottomLabel) {
    var g = new THREE.Group();
    var h1 = 0.98, h2 = 0.95;
    var bottom = makeFrontLoad(width, h1, depth, bodyWhite, 'rect', undefined, bottomNumber, bottomLabel);
    g.add(bottom);
    var top = makeFrontLoad(width, h2, depth, bodyWhite, 'rect', undefined, topNumber, topLabel);
  ...
  for (var wi = 0; wi < washerWidths.length; wi++) {
    ...
    if (wi < 2) {
      ...
      unit = makeTopLoad(tlW, tlH, tlD, bodyAlmond, washerNumber, washerLabel);
    }
  ...
  var standaloneDryer = makeFrontLoad(dfW, 1.0, dfD, bodyWhite, 'rect', undefined, 6, 'DRYER 6');
  ...
  // cabinet top (1.0) and the top of the wall (WALL_H, 3.0).
  var poster = makePoster(0.9144, 1.2192);
  poster.rotation.y = -Math.PI / 2;
  poster.position.set(ROOM_W - 0.02, 2.0, dryerCenters[0]);
  scene.add(poster);
