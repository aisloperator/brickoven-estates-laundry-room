(function () {
  ...
  // original smaller label + "FRONT-LOADING" subtitle, while every other
  ...
  function createLogoTexture(label, showCommercial, mainFontSize) {
    ...
    if (showCommercial) {
      ...
      ctx.fillText('F R O N T - L O A D I N G', 256, 92);
    }
    var tex = new THREE.CanvasTexture(cnv);
    tex.anisotropy = 4;
    tex.encoding = THREE.sRGBEncoding;
  ...
  function heDetergentPosterTexture() {
    ...
    return createPosterTexture('#e3f5df', '#9dcf94', [
      { text: 'The front-loading', y: 122, color: '#1a56c4', font: blue },
      { text: 'washers are High', y: 167, color: '#1a56c4', font: blue },
      { text: 'Efficiency', y: 212, color: '#1a56c4', font: blue },
      { text: 'Use only High Efficiency', y: 277, color: '#2a2a28', font: POSTER_SMALL },
      { text: 'detergent, and only the', y: 307, color: '#2a2a28', font: POSTER_SMALL },
      { text: 'specified amount, to', y: 337, color: '#2a2a28', font: POSTER_SMALL },
      { text: 'avoid residue in clothes', y: 367, color: '#2a2a28', font: POSTER_SMALL },
      { text: 'and machines.', y: 397, color: '#2a2a28', font: POSTER_SMALL }
    ]);
  }
  ...
  // Same coplanar-face issue as the jamb posts above, but horizontal: this
  // trim's bottom face used to land exactly at y=doorHeight, precisely
  // where the brick header box's own bottom face already sits. Dropping
  // it 0.006 lower (past doorHeight, into the open doorway) breaks the tie
  // the same way the jambs' inner-edge nudge does.
  trimBox((doorGapStart + doorGapEnd) / 2, doorHeight + 0.037, -WALL_T / 2, (doorGapEnd - doorGapStart) + 0.16, 0.086, WALL_T + 0.04);
  ...
  function drawStatusCanvas(ctx, minutes) {
    ...
    ctx.textBaseline = 'middle';
    if (minutes < 0) {
      // Negative (currently only -1, an error/unknown-status placeholder
      // fed in by "Random Fake Data") reads as a red X rather than either
      // the idle checkmark or a running-machine minutes count.
  ...
  // X); otherwise, 0.5 chance it reads idle (0 minutes), otherwise a random
  // run length — washers 1-35 minutes, dryers 0-60 (dryers can independently
  // land on 0 from that branch too, on top of the shared 0.5 idle chance,
  ...
  function randomFakeData() {
    for (var num = 1; num <= 10; num++) {
      ...
      if (Math.random() < 0.1) {
        ...
        minutes = Math.random() < 0.5 ? 0 : (isWasher ? (1 + Math.floor(Math.random() * 35)) : Math.floor(Math.random() * 61));
      }
  ...
  function makeTopLoad(width, height, depth, bodyMat, machineNumber, label) {
    ...
    // "FRONT-LOADING" subtitle, same as the dryers.
    var decal = decalPlane(width * 0.75, height * 0.1, label, false, 58);
    decal.position.set(0, consoleY, consoleZ + 0.045);
    g.add(decal);

    // Centered on the cabinet box itself (tubY/2), not the console/lid
  ...
  // washer wall), and a fourth further along the x=0 edge (the other side
  // of the corner from that third stack). This whole area is well clear of
  // every appliance regardless of x, since no machine's footprint extends
  // anywhere near z=ROOM_D. Pushed right up against their respective edges
  // (small edgeInset, just enough to clear a stack's own radius) and
  // spaced at half their original separation.
  var edgeInset = 0.2;
  var cornerStackSpecs = [
    ...
    { x: edgeInset, z: ROOM_D - edgeInset - 0.58, count: 22 }  // further along the x=0 edge
  ];
  ...
  var PAGE_FONT_SMALL = 'bold 18px Georgia, serif';
  var leftPageMat = new THREE.MeshBasicMaterial({
    map: createPosterTexture('#f0e6c8', '#cbb98f', [
      { text: 'Books and videos in', y: 106, color: '#2a2216', font: PAGE_FONT },
      { text: "the laundry room's", y: 152, color: '#2a2216', font: PAGE_FONT },
      { text: 'bookshelves corner', y: 198, color: '#2a2216', font: PAGE_FONT },
      { text: 'are free.', y: 244, color: '#2a2216', font: PAGE_FONT },
      { text: 'You can leave books', y: 314, color: '#2a2216', font: PAGE_FONT },
      { text: 'and videos here for', y: 360, color: '#2a2216', font: PAGE_FONT },
      { text: 'neighbors to reuse.', y: 406, color: '#2a2216', font: PAGE_FONT }
    ])
  });
  var rightPageMat = new THREE.MeshBasicMaterial({
    map: createPosterTexture('#f0e6c8', '#cbb98f', [
      { text: 'You can also leave free', y: 88, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'small, non-clothing items', y: 118, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'on the corner of the', y: 148, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'countertop next to the', y: 178, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: "bookshelves, if there's", y: 208, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'room.', y: 238, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: "And it's a local tradition", y: 282, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'to put usable unwanted', y: 312, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'household items on the', y: 342, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'sidewalk curb, for a', y: 372, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'passerby to reuse.', y: 402, color: '#2a2216', font: PAGE_FONT_SMALL },
      { text: 'Clothing is best donated.', y: 446, color: '#2a2216', font: PAGE_FONT_SMALL }
    ])
  });
